...
#仮引数：表示するレイヤー画像、二値化の閾値、輪郭線のカラー、輪郭線の幅、レイヤー画像の表示切替、背景の表示切替、背景カラー
    ...
    if isinstance(color, str) and color.startswith("#"):
        ...
        b = int(color[4:6], 16)
        color = (b, g, r)
    if isinstance(background2, str) and background2.startswith("#"):
        background2 = background2.lstrip('#')
        r = int(background2[0:2], 16)
...
if selected == 'ツールを使用する':
    ...
    if uploaded_file is not None:
        ...
        if empty_pixels == total_pixels:
            ...
            #========================================
            st.markdown('''
            <p class='my-text'>線画レイヤの選択</p>
            
            #### 一覧から線画レイヤを選択してください。''', unsafe_allow_html=True)
            name_list = []
            img_list = []
            layer_number = 0
            #入力ファイルが変更されたかを判定
            ...
            if functions:
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    for function in functions:
                        ...
                        if function == 'SelectionColor_Display_System':
                            st.markdown('''
                            <p class='my-text'>指定色表示システム</p>
                            <p class='box'>表示する色を選択</p>''', unsafe_allow_html=True)
                            ...
                            background1 = st.color_picker('###### 出力画像の背景色が変化します。', '#FFFFFF', key='back1')
                            #機能の実行
                            result_img1 = SelectionColor_Display_System(psd, colors, thresholds, background1)
                            st.markdown('''<p class='box2'>指定色表示システムの出力結果</p>''', unsafe_allow_html=True)
                            #出力画像の表示
                            ...
                            result_buf1.seek(0)
                            #zipファイルに追加
                            zip_file.writestr('指定色表示システム.png', result_buf1.getvalue())
                    
                        #指定レイヤ輪郭表示システム
                        elif function == 'SelectionLayers_Contour_System':
                            st.markdown('''
                            <p class='my-text'>指定レイヤ輪郭表示システム</p>
                            ...
                            layer_display_switching = st.radio('###### レイヤの表示・非表示を切り替えられます。', ('表示', '非表示'), horizontal=True)
                            background_display_switching = st.radio('###### 背景色の表示・非表示を切り替えられます。（非表示の場合、背景は透過されます。）', ('表示', '非表示'), horizontal=True)
                            st.markdown('''<p class='box'>各素材の色を選択</p>''', unsafe_allow_html=True)
                            support_color1 = st.color_picker('###### 輪郭線の色が変化します。', '#00FF00')
                            background2 = st.color_picker('###### 出力画像の背景色が変化します。', '#FFFFFF', key='back2')
                            st.markdown('''<p class='box'>表示する素材の選択</p>''', unsafe_allow_html=True)
                            #セッションの初期化（表示レイヤの特定と合成用）
                            ...
                            st.write('###### :red[＊指定がない場合、線画レイヤが自動で選択されます。]')
                            ...
                            result_buf2.seek(0)
                            #zipファイルに追加
                            zip_file.writestr('指定レイヤ輪郭表示システム.png', result_buf2.getvalue())
                        ...
                        #消し忘れ検知システム２
                            ...
                            if result_img6 == 0:
                                st.error('線画レイヤが選択されていない、またはエラーイラストが入力されました。')
                            else:
                ...
                #ダウンロードボタンの表示
                st.download_button(
                    label="すべての出力画像をダウンロード",
                    data=zip_buffer, 
                    file_name="output_results.zip",
                    mime="application/zip"
                )
            else:
                st.error('使用する機能が選択されていません。')